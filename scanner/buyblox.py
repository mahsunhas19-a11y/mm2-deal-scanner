...
def scan_buyblox(valid_names=None, show_items=False):
    with sync_playwright() as p:
        ...
        try:
            page.goto(
                "https://buyblox.gg/collections/mm2",
                wait_until="domcontentloaded",
                timeout=60000,
            )
            page.wait_for_timeout(5000)
            print("BuyBlox geöffnet.")
            ...
            cards = page.locator("div.card.product-card.product-card--card")
            if cards.count() == 0:
                cards = page.locator("div.mcp-grid-card")
            ...
            weapons, audit_rows = match_catalog("BuyBlox", raw_items, valid_set or {x["shop_title"] for x in raw_items})
            save_match_audit("buyblox", audit_rows)
            if show_items:
                print("=" * 60)
                print("SHOP: BUYBLOX")
                print("=" * 60)
                ...
                print()
            print(f"BuyBlox insgesamt: {len(weapons)}")
            return weapons
        finally:
            browser.close()
